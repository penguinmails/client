    ...
    <div className="flex items-center justify-end space-x-2">
      ...
      <AlertDialogDelete
        ...
        onDelete={async () => {
          ...
          developmentLogger.debug(`Deleting mailbox: ${mailbox.email}`);
        }}
      />
    </div>
  );
...
function UpdateMailboxSetting({ mailbox }: { mailbox: Mailbox }) {
  const form = useForm<updateMailboxValuesType>({
    ...
    defaultValues: {
      ...
      newPassword: "",
      confirmPassword: "",
      pause: mailbox.warmupStatus === "WARMING", // Assuming warming means not paused
    },
  });

  const onSubmit = (values: updateMailboxValuesType) => {
    developmentLogger.debug("Submitting mailbox form", values);
  };
