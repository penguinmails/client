...
async function handleDeleteCampaign(id: number): Promise<void> {
  ...
  // Note: In client components, we can't pass NextRequest, so we'll need to handle this differently
  // For now, we'll create a wrapper API route or modify the server action
  try {
    const result = await fetch(`/api/campaigns/${id}`, { method: 'DELETE' });
    if (!result.ok) {
    ...
    productionLogger.error("Error deleting campaign:", id, "\n", error);
...
async function handlePauseCampaign(id: number): Promise<void> {
  try {
    const result = await fetch(`/api/campaigns/${id}/pause`, { method: 'POST' });
    if (!result.ok) {
    ...
    productionLogger.error("Error pausing campaign:", id, "\n", error);
...
async function handleResumeCampaign(id: number): Promise<void> {
  try {
    const result = await fetch(`/api/campaigns/${id}/resume`, { method: 'POST' });
    if (!result.ok) {
    ...
    productionLogger.error("Error resuming campaign:", id, "\n", error);
...
async function handleDuplicateCampaign(id: number): Promise<void> {
  try {
    ...
    productionLogger.error("Error duplicating campaign:", id, "\n", error);
