...
function MigratedTableRow({ campaign }: { campaign: CampaignDisplay }) {
  return (
...
export interface MigratedCampaignsTableProps {
...
/**
 * MigratedCampaignsTable - Campaigns table matching legacy visual appearance
 * 
 ...
 */
export function MigratedCampaignsTable({
      ...
      <CardContent className="overflow-x-auto p-0">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-border">
            {data.length > 0 ? (
              data.map((campaign) => (
                <MigratedTableRow key={campaign.id} campaign={campaign} />
              ))
            ) : (
              <tr>
...
export default MigratedCampaignsTable;
