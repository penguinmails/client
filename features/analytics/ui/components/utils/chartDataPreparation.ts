...
import { ChartDataPoint, AnalyticsUIFilters } from "@features/analytics/types/ui";
 ...
 */
export function convertUIFiltersToDataFilters(filters: UIFilters | AnalyticsUIFilters | unknown) {
  ...
  // Type assertion to handle both filter types
  const typedFilters = filters as UIFilters & AnalyticsUIFilters;

  return {
    dateRange: {
      start: typedFilters.customDateRange?.start || 
        new Date(Date.now() - getDaysFromRange(typedFilters.dateRange) * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
      end: typedFilters.customDateRange?.end || new Date().toISOString().split("T")[0],
    },
    entityIds: typedFilters.selectedCampaigns.length > 0 ? typedFilters.selectedCampaigns : undefined,
  };
}
