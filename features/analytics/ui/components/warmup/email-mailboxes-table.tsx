  ...
  useEffect(() => {
    ...
    const fetchAllAnalytics = async () => {
      try {
        ...
        mailboxes.forEach((mailbox) => {
          const raw =
            ...
            typeof analyticsResultsRawUnknown === "object"
              ...
              : null;
          const legacy = raw && typeof raw === 'object' && Object.keys(raw).length > 0 
            ? mapRawToLegacyMailboxData(raw as Record<string, unknown>) 
            : null;
          if (legacy) {
            newState[mailbox.id] = {
              data: legacy,
        ...
        productionLogger.error("Failed to fetch mailbox analytics:", error);
