...
import { UserPreferencesResponse } from '@features/settings/types/user';
...
export function ClientPreferencesProvider({ children }: ClientPreferencesProviderProps) {
  ...
  const refreshPreferences = async () => {
    try {
      ...
      const validatedData = userPreferencesSchema.parse(preferencesData) as UserPreferencesResponse;
      setPreferences(validatedData);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to load preferences';
      setError(message);
      productionLogger.error('Error loading preferences:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const updatePreferences = async (_updates: Partial<UserPreferencesResponse>): Promise<void> => {
    try {
      ...
      setError(message);
      productionLogger.error('Error updating preferences:', err);
      throw err;
    }
  };
...
export function usePreferences() {
  developmentLogger.warn('usePreferences is deprecated. Use useClientPreferences instead.');
