...
export async function sendVerificationEmail(data: z.infer<typeof sendVerificationEmailSchema>) {
  try {
    ...
    productionLogger.error('Send verification email error:', error);
    throw error;
  }
}
...
export async function sendPasswordResetEmail(data: z.infer<typeof sendPasswordResetEmailSchema>) {
  try {
    ...
    productionLogger.error('Send password reset email error:', error);
    throw error;
  }
}
...
export async function sendWelcomeEmail(data: z.infer<typeof sendWelcomeEmailSchema>) {
  try {
    ...
    productionLogger.error('Send welcome email error:', error);
    throw error;
  }
}
...
export async function sendTransactionalEmail(
  ...
  try {
    ...
    productionLogger.error('Send transactional email error:', error);
    throw error;
  }
}