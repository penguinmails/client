...
function getFileLayer(filePath: string): string | null {
  ...
  if (filePath.includes("/features/") || filePath.startsWith("features/"))
    return "features";
  if (filePath.includes("/shared/") || filePath.startsWith("shared/"))
    return "shared";
  if (
...
function getFeatureName(path: string): string | null {
  ...
  const featureMatch =
    path.match(/\/features\/([^\/]+)/) || path.match(/features\/([^\/]+)/);
...
const isSameFeature =
  currentFeature && importFeature && currentFeature === importFeature;
